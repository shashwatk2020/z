...
const IcoToPngConverter = () => {
  ...
  const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const icoFiles = files.filter(file => 
      file.name.toLowerCase().endsWith('.ico') || file.type === 'image/x-icon'
    );
    
    if (icoFiles.length !== files.length) {
      toast({
        title: "Invalid Files",
        description: "Please select only ICO files.",
        variant: "destructive"
      });
    }
  ...
  const convertToPng = async () => {
    if (selectedFiles.length === 0) {
      toast({
        ...
        description: "Please select ICO files to convert.",
        variant: "destructive"
      });
      return;
    }
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-gray-600 max-w-3xl mx-auto">
            Convert Windows ICO icon files to PNG format for universal compatibility and web use.
          </p>
        </div>
        ...
        <div className="grid gap-8">
          <Card>
            <CardHeader>
              <CardTitle>Upload ICO Files</CardTitle>
              <CardDescription>Select one or more ICO files to convert to PNG format</CardDescription>
            </CardHeader>
...
export default IcoToPngConverter;

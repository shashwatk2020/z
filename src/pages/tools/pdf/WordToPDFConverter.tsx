...
const WordToPDFConverter = () => {
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && (file.type === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' || file.type === 'application/msword')) {
      ...
      toast({
        title: "Invalid File",
        description: "Please select a Word document (.doc, .docx).",
        variant: "destructive"
      });
    }
  };
    ...
    <Layout>
      <div className="max-w-4xl mx-auto p-6 space-y-6">
        <div className="text-center space-y-2">
          ...
          <p className="text-gray-600">Convert your Word documents to PDF files</p>
        </div>

        <Card>
          <CardHeader>
            <CardTitle>Select Word File</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <Button onClick={() => fileInputRef.current?.click()} variant="outline" className="w-full">
              <Upload className="h-4 w-4 mr-2" />
              {wordFile ? `Selected: ${wordFile.name}` : 'Choose Word File'}
            </Button>
            <input 
              ref={fileInputRef} 
              type="file" 
            ...
            {wordFile && (
              <div className="p-4 bg-gray-50 rounded-lg">
                <p className="text-sm">
                  <strong>File:</strong> {wordFile.name} ({(wordFile.size / 1024 / 1024).toFixed(2)} MB)
                </p>
              </div>
            )}
          </CardContent>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <Button onClick={startConversion} disabled={isConverting || !wordFile} className="w-full">
        ...
        {convertedPdfUrl && (
          <Card>
            ...
            <CardContent className="space-y-4">
              <p className="text-sm text-gray-600">Your Word document has been converted to a PDF file.</p>
              <Button asChild className="w-full">
                <a href={convertedPdfUrl} download={`${wordFile?.name.replace(/\.(doc|docx)$/, '.pdf')}`}>
                  <Download className="h-4 w-4 mr-2" />
                  Download PDF
                </a>
              </Button>
